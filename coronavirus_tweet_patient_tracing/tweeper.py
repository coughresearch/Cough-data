...
async def getUrl(init):
    '''
    URL Descision:
    Tweeper utilizes positions of Tweet's from Twitter's search feature to
...
async def getFeed(init):
    ...
    try:
        ...
        # Tweeper will realize that it's done scraping.
        pass
...
async def outTweet(tweet):
    ...
    '''
    There were certain cases where I used Tweeper
    to gather a list of users and then fed that
    generated list into Tweeper. That's why these
    modes exist.
    '''
    if arg.users:
        output = username
...
async def main():
    ...
    while True:
        ...
        with, telling Tweeper it's finished scraping.
        '''
        if len(feed) > 0:
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(prog="tweeper.py", usage="python3 %(prog)s [options]", description="tweeper.py - An Advanced Twitter Scraping Tool")
